...
class CodeParser:
    ...
    def _resolve_call_targets(
        ...
        for edge in edges:
            ...
            # JS/TS calls retain their complete static member path as
            # evidence. Resolve it before receiver-based handling, which
            # deliberately leaves ordinary member calls otherwise bare.
            ...
            if (
                ...
                member_candidates = [
                    qualified
                    for qualified, _ in candidate_entries(member_call, edge.kind)
                ]
                if member_candidates:
                    edge = EdgeInfo(
                        kind=edge.kind,
                        source=edge.source,
                        target=member_candidates[0],
                        file_path=edge.file_path,
                        line=edge.line,
                        extra=edge.extra,
                    )
    ...
    def _extract_js_member_functions(
        ...
        """Extract static JS/TS member assignments whose value is a function."""
        assign = next(
            (
                sub for sub in child.children
        ...
        if (
            ...
            or right.type not in self._JS_FUNC_VALUE_TYPES
            # A function-local assignment has no stable module-level member
            # identity, and sibling scopes could otherwise collide.
            or enclosing_func is not None
        ):
            return False
        ...
        nodes.append(NodeInfo(
            ...
            return_type=self._get_return_type(right, language, source),
            is_test=is_test,
        ))
        edges.append(EdgeInfo(
            kind="CONTAINS",
    ...
    def _get_js_member_call_name(node) -> Optional[str]:
        """Return a static JS/TS member call path for same-file resolution."""
