...
def review_changes_prompt(base: str = "HEAD~1") -> list[Message]:
    ...
    return _user(
        ...
        f'`detect_changes_tool(base="{base}", detail_level="minimal")` → report summary '
        f"+ any test gaps.\n"
        f'3. If risk is "medium" or "high":\n'
        f'   a. Call `detect_changes_tool(base="{base}", detail_level="standard")` for '
        f"full change list.\n"
        f"   b. For each high-risk function, call "
        f'`query_graph_tool(pattern="callers_of", target=<func>, '
        f'detail_level="minimal")`.\n'
        f'   c. Call `get_affected_flows_tool(base="{base}", detail_level="minimal")` '
...
def pre_merge_check_prompt(base: str = "HEAD~1") -> list[Message]:
    ...
    return _user(
        ...
        f'1. Call `get_minimal_context_tool(task="pre-merge check", base="{base}")`.\n'
        f'2. Call `detect_changes_tool(base="{base}", detail_level="minimal")` for risk '
        "score and test gaps.\n"
        "3. If risk > 0.4: call "
        f'`get_affected_flows_tool(base="{base}", detail_level="minimal")`.\n'
