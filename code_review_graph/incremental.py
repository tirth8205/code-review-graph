...
def find_project_root(start: Path | None = None) -> Path:
    ...
    If the ``CRG_REPO_ROOT`` environment variable is set its value is returned
    directly, allowing the MCP server to be anchored to a specific repository
    without relying on the process working directory.  This is useful when
    running ``code-review-graph serve`` from a launcher (e.g. an IDE or a
    user-level MCP config) where the CWD is not the repository root.
    """
    crg_repo = os.environ.get("CRG_REPO_ROOT")
    if crg_repo:
        return Path(crg_repo)
...
def get_data_dir(repo_root: Path) -> Path:
    """Return the data directory for generated files (graph DB, wiki, HTML).

    By default this is ``.code-review-graph/`` inside the repository root.
    Set ``CRG_DATA_DIR`` to an absolute path to store all generated files
    outside the repository, keeping the working tree completely clean.

    If ``CRG_DATA_DIR`` is a relative path it is resolved relative to
    ``repo_root``.
...
def get_db_path(repo_root: Path) -> Path:
    ...
    Uses :func:`get_data_dir` to locate the data directory so the database
    can be stored outside the repository via ``CRG_DATA_DIR``.

    When using the default in-repo location, creates the
    ``.code-review-graph/`` directory with an inner ``.gitignore`` (``*``)
    so generated files are never accidentally committed.  If a legacy
    ``.code-review-graph.db`` exists at the repo root the database is
    migrated into the new directory (WAL/SHM side-files are discarded).
    """
    ...
    # Only write the .gitignore when using the default in-repo location
    # (an external directory managed by the user should not be modified).
    if not os.environ.get("CRG_DATA_DIR"):
