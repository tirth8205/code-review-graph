...
<p align="center">
  ...
  <a href="docs/ROADMAP.md">لائحۂ عمل</a>
</p>

<br>

AI کوڈنگ ٹولز جائزے کے کام میں آپ کے کوڈ بیس کے بڑے حصوں کو دوبارہ پڑھ سکتے ہیں۔ `code-review-graph` یہ مسئلہ حل کرتا ہے۔ یہ [Tree-sitter](https://tree-sitter.github.io/tree-sitter/) کے ساتھ آپ کے کوڈ کا ایک ساختی نقشہ بناتا ہے، تبدیلیوں کو قدم بہ قدم ٹریک کرتا ہے، اور [MCP](https://modelcontextprotocol.io/) کے ذریعے آپ کے AI معاون کو درست سیاق و سباق فراہم کرتا ہے تاکہ وہ صرف وہی پڑھے جو اہم ہو۔
...
ایک کمانڈ سب کچھ ترتیب دے دیتی ہے۔ `install` خود اس بات کی شناخت کرتی ہے کہ آپ کے پاس کون سے AI کوڈنگ ٹولز موجود ہیں، ہر ایک کے لیے درست MCP ترتیب لکھتی ہے، جہاں معاونت موجود ہو وہاں پلیٹ فارم کے مقامی hooks/skills انسٹال کرتی ہے، اور آپ کے پلیٹ فارم کے قواعد میں گراف سے آگاہ ہدایات شامل کرتی ہے۔ یہ خودکار طور پر معلوم کرتی ہے کہ آپ نے `uvx` یا `pip`/`pipx` کے ذریعے انسٹال کیا ہے اور مناسب ترتیب تیار کرتی ہے۔ انسٹال کے بعد اپنے ایڈیٹر/ٹول کو دوبارہ شروع کریں۔

<p align="center">
  <img src="diagrams/diagram8_supported_platforms.png" alt="ایک انسٹال، ہر پلیٹ فارم: Codex، Claude Code، CodeBuddy Code، Cursor، Windsurf، Zed، Continue، OpenCode، Antigravity، Gemini CLI، Qwen، Qoder، Kiro، GitHub Copilot، اور GitHub Copilot CLI کی خودکار شناخت اور ترتیب" width="85%" />
</p>
...
کسی Git یا SVN پروجیکٹ سے CRG ہٹانے کے لیے پروجیکٹ کی working tree کے اندر کسی بھی
جگہ سے متعلقہ uninstall کمانڈ چلائیں۔ ہدف کو working tree کی جڑ تک normalize کیا
جاتا ہے اور ایسی directories قبول نہیں کی جاتیں جو repository نہ ہوں۔ یہ صرف
CRG کی اپنی فائلیں اور entries ہٹاتی ہے؛ دوسرے MCP servers، hooks، skills اور
JSONC comments کو نہیں چھیڑا جاتا۔ مشترکہ configuration میں تبدیلی atomic
replacement سے ہوتی ہے، اس لیے write ناکام ہو تو اصل فائل محفوظ رہتی ہے۔
...
جب کوئی فائل بدلتی ہے، تو گراف ہر ایسے caller، dependent اور test کو تلاش کرتا ہے جو متاثر ہو سکتا ہے۔ یہ تبدیلی کا "بلاسٹ ریڈیئس" ہے۔ آپ کا AI پورے پروجیکٹ کو اسکین کرنے کے بجائے صرف ان فائلوں کو پڑھتا ہے۔
...
جب hooks یا watch mode فعال ہوں، تو فائل محفوظ ہونے اور معاونت یافتہ commit hooks سے incremental updates شروع ہو جاتے ہیں۔ گراف بدلی ہوئی فائلوں کا diff لیتا ہے، SHA-256 hash checks کے ذریعے ان کے dependents تلاش کرتا ہے، اور صرف بدلی ہوئی فائلوں کو دوبارہ پارس کرتا ہے۔ 2,900 فائلوں پر مبنی پروجیکٹ 2 سیکنڈ سے کم میں دوبارہ انڈیکس ہو جاتا ہے۔
...
بڑی مونو ریپوزٹریوں میں ٹوکن کا ضیاع سب سے زیادہ محسوس ہوتا ہے۔ گراف شور کو چھانٹ دیتا ہے — 27,700+ فائلوں کو جائزے کے سیاق و سباق سے خارج کر دیا جاتا ہے، صرف تقریباً 15 فائلیں واقعی پڑھی جاتی ہیں۔

<p align="center">
  <img src="diagrams/diagram6_monorepo_funnel.png" alt="code-review-graph ریپوزٹری: 208,821 سورس ٹوکنس ایک چھوٹے سے گراف جواب میں تقریباً 2,495 ٹوکن تک آتے ہیں — سوال کے لیے 93x کم ٹوکنس" width="80%" />
</p>
...
PHP پروجیکٹس میں ریپوزٹری تک محدود Composer PSR-4 resolution، Blade template
references، اور Laravel Route/Eloquent semantic edges کی اضافی سہولت بھی ملتی
ہے—مگر صرف اس وقت جب سورس میں واضح framework imports، model inheritance اور
receiver evidence موجود ہو۔

### اپنی زبان شامل کریں (کوئی فورک درکار نہیں)

اگر آپ کے ریپو میں ایسی زبان استعمال ہوتی ہے جس کی پارسر میں ابھی تک معاونت موجود نہیں، تو `.code-review-graph/` میں ایک `languages.toml` فائل ڈالیں جو فائل ایکسٹینشنز کو `tree_sitter_language_pack` میں موجود کسی بھی گرامر سے اور فنکشنز، کلاسز، امپورٹس، اور کالز کے لیے tree-sitter node types سے مربوط کرے:
...
عام tree-sitter walker اس سے ضروری معلومات نکالتا ہے — کوئی کوڈ تبدیلی نہیں، اور built-in زبانوں کو override نہیں کیا جا سکتا۔ schema reference، validation rules اور ایک مکمل مثال کے لیے [docs/CUSTOM_LANGUAGES.md](docs/CUSTOM_LANGUAGES.md) دیکھیں۔

### CI میں رسک-اسکور شدہ PR جائزے (GitHub Action)

یہی تجزیہ ایک composite GitHub Action کے طور پر بھی چلتا ہے — اور یہ مقامی طور پر ہی رہتا ہے: knowledge graph آپ کے CI runner پر ہی بنایا جاتا اور وہیں اس سے استفسار کیا جاتا ہے، جبکہ سورس کوڈ کسی بیرونی سروس کو نہیں بھیجا جاتا۔ ہر pull request پر یہ ایک sticky comment پوسٹ کرتا ہے جس میں رسک-اسکور شدہ فنکشنز، متاثرہ execution flows اور ٹیسٹ کوریج میں خلا شامل ہوتے ہیں، اور ہر push پر اسی جگہ اپ ڈیٹ ہوتے رہتے ہیں۔ ایک اختیاری `fail-on-risk` input اس جائزے کو merge gate بھی بنا سکتا ہے۔
...
<p align="center">
  <img src="diagrams/diagram5_benchmark_board.png" alt="6 حقیقی ریپوزٹریوں پر بینچمارکس: ~82x میڈین per-question ٹوکن کمی (528x زیادہ سے زیادہ)، graph-derived ground truth کے مقابل 0.71 اوسط impact F1" width="85%" />
</p>

**اہم نمبر: 6 ریپوزٹریوں میں per-question ٹوکن کمی کا میڈین ~82x ہے** (پورے corpus baseline کے مقابلے میں graph query)۔ اکثر دہرایا جانے والا **528x** زیادہ سے زیادہ ہے — ایک واحد بہترین case repo (fastapi) — عام نتیجہ نہیں۔
...
<details>
<summary><strong>ٹوکن کارکردگی: ~82x میڈین per-question کمی (حد 38x – 528x؛ پورے کورپس کے مقابلے میں گراف استفسار)</strong></summary>
<br>

ایک عام ایجنٹ سوال (مثلاً `"how does authentication work"`، `"what is the main entry point"` وغیرہ) کے لیے، گراف مخصوص سرچ ہٹس اور نیبر ایجز سے تقریباً 2,000–3,500 ٹوکن واپس کرتا ہے، اس کے بجائے کہ ایجنٹ کو پورے سورس کوڈ کو پڑھنا پڑے۔ درج ذیل جدول ان 5 نمونہ سوالات کا اوسط دکھاتا ہے جو `code_review_graph/token_benchmark.py` میں تعریف کیے گئے ہیں۔
...
اوپر دکھایا گیا whole-corpus baseline ایسی بالائی حد ہے جو حقیقی agent کے استعمال کی نمائندگی نہیں کرتی: ایک قابل agent identifiers تلاش کرے گا اور صرف سب سے زیادہ ملنے والی فائلوں کو پڑھے گا۔ `agent_baseline` eval benchmark یہ حقیقت پسندانہ baseline ماپتا ہے — corpus پر pure-Python grep، match count کے مطابق سب سے اوپر 3 فائلیں، token count، اور graph query کی لاگت سے موازنہ (`evaluate/results/<repo>_agent_baseline_*.csv`)۔
...
v2.3.4 کے بعد سے، review اور impact ٹولز ایک کمپیکٹ `context_savings` تخمینہ بھی لگاتے ہیں تاکہ MCP کلائنٹس یہ دیکھ سکیں کہ ہر کال پر تقریباً کتنا سیاق و سباق بچایا گیا۔ v2.3.5 میں CLI اس کو اوپر دکھائے گئے باکسڈ `Token Savings` پینل کے طور پر ظاہر کرتی ہے (استعمال کے حصے میں "ٹوکن بچت پینل" دیکھیں) اور `--verify` شامل کرتی ہے تاکہ OpenAI کے `cl100k_base` tokenizer کے مقابلے میں جانچ کی جا سکے۔ [`docs/REPRODUCING.md`](docs/REPRODUCING.md) میں موجود calibration data سے ظاہر ہوتا ہے کہ یہ تخمینہ 222 نمونہ فائلوں پر مجموعی طور پر حقیقی GPT-4 ٹوکن سے تقریباً 1% کے اندر ہے۔
...
بلاسٹ ریڈیئس تجزیہ 13 eval commits پر ground truth میں موجود ہر فائل کو حاصل کر لیتا ہے — **لیکن اسے ایک upper bound سمجھ کر پڑھیں، نہ کہ "100% recall" کے طور پر**: اس موڈ میں ground truth (بدلی ہوئی فائلیں + وہ فائلیں جن کے call/import edges ان تک آتے ہیں) خود اسی گراف سے اخذ کی گئی ہے جس پر predictor چلتا ہے، لہٰذا یہ by construction circular ہے۔ precision کالم میں نظر آنے والی over-prediction ایک جان بوجھ کر اختیار کی گئی trade-off ہے: خراب dependency چھوٹ جانے سے بہتر ہے کہ زیادہ فائلوں کو flag کیا جائے۔
...
یہ benchmark ایک دیانت دار **co-change mode** بھی چلاتا ہے: predictor کو ایک بدلی ہوئی فائل سے شروع کیا جاتا ہے اور اسے ان *دوسری* فائلوں کے مقابل grade کیا جاتا ہے جنہیں مصنف نے واقعی اسی commit میں چھوا تھا — یہ git history سے حاصل ہونے والا نسبتاً آزاد ثبوت ہے، گراف سے نہیں۔ دونوں modes result CSVs میں ساتھ دکھائے جاتے ہیں (`ground_truth_mode` کالم)۔ canonical stats میں co-change numbers تب شامل کیے جائیں گے جب eval runner انہیں حاصل کر لے؛ ہم انہیں ماپنے سے پہلے بیان نہیں کرتے۔

</details>

<details>
<summary><strong>تعمیر کی کارکردگی</strong></summary>
<br>
...
- **Flow detection (33% recall):** framework اور روایتی entry patterns کی پہچان Python اور PHP/Laravel میں سب سے مضبوط ہے۔ JavaScript اور Go کے لیے flow detection میں اب بھی بہتری درکار ہے۔
...
| **Framework-aware PHP parsing** | ریپوزٹری تک محدود Composer PSR-4 imports، Blade template references، اور ثبوت کی بنیاد پر Laravel Route-to-controller اور Eloquent relationship edges |
...
| **Semantic search** | sentence-transformers، Google Gemini، MiniMax، یا کسی بھی OpenAI-compatible endpoint (اصل OpenAI، Azure، new-api، LiteLLM، vLLM، LocalAI) کے ذریعے اختیاری vector embeddings |
...
| **Graph traversal** | کسی بھی node سے free-form BFS/DFS exploration، قابلِ ترتیب depth اور token budget |
...
| **Multi-repo daemon** | `crg-daemon` کئی ریپوزٹریوں کو child processes کے طور پر دیکھتا ہے، health checks اور خودکار restart کے ساتھ |
...
JSON exports مقامی graph data directory کے اندر رہتی ہیں، جسے Git پہلے سے
ignore کرتا ہے۔ ان میں absolute paths اور code-structure metadata شامل ہو سکتا
ہے، اس لیے کسی export کو اپنی مشین سے باہر publish کرنے سے پہلے اس کا جائزہ لے
کر حساس معلومات صاف کریں۔
...
ان میں سے کسی ایک کمانڈ میں `--verify` شامل کریں تاکہ دکھائے جانے والے اعداد و شمار کا OpenAI کے `cl100k_base` tokenizer سے موازنہ کیا جا سکے (GPT-4 خاندان)۔ اس کے لیے `pip install tiktoken` کی ضرورت ہے۔ عام change set پر تخمینہ حقیقی ٹوکن سے تقریباً 1% کے اندر رہتا ہے — calibration data کے لیے [`docs/REPRODUCING.md`](docs/REPRODUCING.md) دیکھیں۔

ایک ہی `context_savings` metadata خود بخود `get_impact_radius`، `get_review_context`، `detect_changes`، اور `get_architecture_overview` MCP tools کے JSON responses میں بھی منسلک ہوتا ہے، تاکہ AI agents کسی اضافی prompting کے بغیر chat میں انسانوں کو ٹوکن کی بچت دکھا سکیں۔
...
اگر آپ کے ایڈیٹر میں hooks کی سپورٹ نہیں ہے (مثلاً Cursor، OpenCode)، یا آپ چاہتے ہیں کہ گراف کسی editor integration کے بغیر پس منظر میں تازہ رہے، تو daemon مناسب ہے۔ یہ آپ کی ریپوزٹریوں میں فائلوں کی تبدیلیاں دیکھتا ہے اور خودکار طور پر گراف دوبارہ بناتا ہے — `build` یا `update` کی دستی کمانڈز کی ضرورت نہیں۔
...
پسِ پردہ، `crg-daemon add` `~/.code-review-graph/watch.toml` نامی TOML config فائل میں لکھتا ہے۔ آپ اس فائل کو براہِ راست بھی ایڈٹ کر سکتے ہیں:
...
Daemon اس config فائل میں تبدیلیاں دیکھتا ہے اور جب repos شامل یا حذف ہوں تو watcher processes خودکار طور پر شروع یا بند کرتا ہے۔ ہر 30 سیکنڈ بعد health checks بند ہو جانے والے watchers کو دوبارہ شروع کرتے ہیں۔ کسی بیرونی dependency کی ضرورت نہیں۔
...
نوٹ: git ریپو میں صرف tracked فائلوں کو انڈیکس کیا جاتا ہے (`git ls-files`)، اس لیے gitignored فائلیں خودکار طور پر چھوڑ دی جاتی ہیں۔ tracked فائلوں کو خارج کرنے کے لیے، یا جب git دستیاب نہ ہو، `.code-review-graphignore` استعمال کریں۔
...
> **ماڈل کے انتخاب کا مشورہ:** `-preview` / `-beta` / `-exp` model IDs سے بچیں
...
> (~10 ٹوکن per node، مثال کے طور پر `"parse_file function (path: str) returns Tree"`)۔ ایسے models جن کا نمایاں معیار long-context body understanding سے آتا ہے (مثلاً Gemini 2 یا Qwen3-8B اپنے MTEB-code SOTA scores پر)، اس مختصر input پر چھوٹے models کے مقابلے میں معیار کا فرق بہت محدود رہے گا۔ Body/docstring embedding کو follow-up enhancement کے طور پر زیرِ نظر رکھا گیا ہے۔
...
CLI flag کو environment variable پر ترجیح حاصل ہے۔ جب دونوں set نہ ہوں، تو تمام tools دستیاب ہوں گے۔ یہ MCP client configurations کے لیے خاص طور پر مفید ہے:
...
[docs/FAQ.md](docs/FAQ.md) میں مختصر، ایماندار جوابات:
...
**source tree** سے انسٹال کرتے وقت (مثلاً `pipx install .`) **PyPI** سے build dependencies درکار ہوتی ہیں (مثلاً `hatchling`)۔ اگر connection warnings کے بعد `Could not find a version that satisfies the requirement hatchling` نظر آئے، تو اس **terminal** کا Python/pip ممکن ہے HTTPS client کے ذریعے `pypi.org` نہ کھول پا رہا ہو (یہ بعض اوقات integrated editor terminal میں، اور کم مواقع پر VPN، firewall یا proxy کی وجہ سے دیکھا جاتا ہے)۔

**اختیارات:**

1. وہی کمانڈ **macOS Terminal.app** (یا iTerm) سے چلائیں، اس کے بعد `pipx install .` یا `pipx install "git+https://..."` دوبارہ کوشش کریں۔
2. **[uv](https://docs.astral.sh/uv/)** سے checkout میں موجود CLI انسٹال کریں (یہ بہت سے معاملات میں `pip` سے مختلف download machinery استعمال کرتا ہے):
...
`code_review_graph/parser.py` میں ترمیم کریں اور `EXTENSION_TO_LANGUAGE` میں اپنی extension شامل کریں، ساتھ ہی `_CLASS_TYPES`، `_FUNCTION_TYPES`، `_IMPORT_TYPES` اور `_CALL_TYPES` میں node type mappings بھی شامل کریں۔ ایک test fixture شامل کریں اور PR کھولیں۔
