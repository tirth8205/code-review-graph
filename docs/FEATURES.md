...
## Unreleased — honest benchmarks (2026-09-16)
- **Headline ratio corrected, and re-based on a realistic baseline.** `run_token_benchmark()` averaged the per-question ratios instead of dividing `naive_corpus_tokens` by the average `graph_tokens`. Because `x -> naive/x` is convex the mean of ratios is always the larger, so every published row was overstated: code-review-graph was quoted as 68.1x where its own columns give 65.5x, and httpx as 60.6x where its columns give 53.5x (2.6% to 13.3% across the six repos). The result dict now carries `avg_graph_tokens`, `answered_questions` and `unanswered_questions`, and `average_reduction_ratio` reproduces from the two columns beside it. `token_efficiency.aggregate()` and `agent_baseline.aggregate()` gained `pooled_*_ratio` (a ratio of totals) in place of the mean of per-row ratios; both keep the median.
- **`agent_baseline` has a published capture at last.** The benchmark shipped in v2.3.8 and had never been run against committed results. 2026-09-16: **median 5.9x** across 18 questions on 6 repos (range 3.5x to 60x, pooled 11.7x). That is now the README headline. The whole-corpus figure (median 50.15x, range 30.8x to 325.6x) is kept and clearly labelled as the upper bound no real agent pays.
- **Every benchmark re-run on current `staging`.** The `token_efficiency` and `multi_hop_retrieval` captures in `evaluate/results/` were all dated 2026-05-25; only `impact_accuracy` had been refreshed. All four, plus `search_quality`, are now captured 2026-09-16 from clean clones at the pinned SHAs. `token_efficiency` improved sharply (code-review-graph `528801f8`: 215,154 graph tokens down to 25,478). `impact_accuracy` is identical to three decimal places. `multi_hop_retrieval` **regressed from 0.909 to 0.727** — published as a regression, with the per-task rank table in [REPRODUCING.md](REPRODUCING.md).
- **The Token Savings panel can report a loss.** `estimate_context_savings()` computed `saved = max(0, baseline - returned)`, so the panel was structurally incapable of showing that graph context cost more than the file it replaced — the usual outcome on a small commit (`token_efficiency` median 0.6x). `saved_tokens` and `saved_percent` are now signed and the panel prints `Cost more:` when they are negative, in the MCP JSON as well as in the CLI. `verify_with_tiktoken()` lost the same clamp.
- **Stale figures corrected across the docs.** README (plus the zh-CN, ja-JP, ko-KR and hi-IN translations), FEATURES, REPRODUCING, FAQ, USAGE and ROADMAP. The FAQ's unsourced "MRR 0.35" for keyword ranking is replaced with the measured 0.847 over the 18 `search_queries` in the eval configs. The dead `#calibration-result-committed` anchor now points at `#calibration-table`.
...
- **`agent_baseline` eval benchmark**: compares graph queries with a grep-and-read-top-3 agent baseline instead of the whole-corpus baseline. Wired into all six pinned eval configs. (First published capture: 2026-09-16, see above.)
...
- **`--verify` flag**: adds a `Verified (tiktoken)` row computed with OpenAI's `cl100k_base` tokenizer. Calibration across 222 files put the aggregate estimate within about 1% of real tokens; see [REPRODUCING.md](REPRODUCING.md#calibration-table).
...
- **`multi_hop_retrieval` benchmark**: 11 hand-curated two-step tool-chain tasks (`hybrid_search` then `query_graph`) across the 6 test repos. Average score 0.909 at that release; 0.727 on the current 2026-09-16 capture.
- **Richer semantic search**: embedding text includes the dotted form (`Module.Class.method`), word-split identifiers, and the enclosing module directory. The multi-hop score rose from 0.545 to 0.909 in that release.
